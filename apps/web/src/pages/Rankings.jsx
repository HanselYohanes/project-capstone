...
// ─── Fallback Data (Hanya jika API bermasalah) ───────────────────────────────
...
// ─── Helpers ────────────────────────────────────────────────────────────────
// Sesuai dengan Postman Hansel yang mengirim status kapital: "CRITICAL"
  ...
  useEffect(() => {
    ...
    const fetchRankings = async () => {
      try {
        ...
        // 🔑 Ambil token JWT dari localStorage agar tidak terkena error 401
        ...
        // 🌐 Tembak endpoint critical-districts sesuai Postman terbaru Hansel
        ...
        if (!cancelled) {
          ...
          if (rawData.length > 0) {
            // 📊 Petakan data bersarang Hansel (_count.violations) agar masuk ke komponen tabel
            const mappedData = rawData.map((d) => ({
              id: d.id,
              name: d.name,
              saturationPercent: parseFloat(d.saturationPercent ?? 0),
              status: d.status || 'STABLE',
              violationCount: d._count?.violations ?? d.violationCount ?? 0 // Proteksi data bersarang
            }));

            // Urutkan dari jumlah pelanggaran tertinggi ke terendah, lalu berikan posisi urutan (Rank)
