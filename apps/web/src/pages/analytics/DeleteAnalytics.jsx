...
/**
 * DeleteAnalytics — Modal konfirmasi hapus data pelanggaran (violation).
 *
 * Props:
 *  - analyticsId   (string)   : ID violation yang akan dihapus
 *  - analyticsName (string)   : Nama/kode untuk tampilan (opsional)
 ...
 */
const DeleteAnalytics = ({ analyticsId, analyticsName, isOpen, onClose, onSuccess }) => {
    ...
    const handleDelete = async () => {
        ...
        try {
            const response = await fetch(`${API_BASE}/violations/${analyticsId}`, {
                method: "DELETE",
            });
            ...
            setTimeout(() => {
                setSuccess(false);
                setError(null);
                if (onSuccess) onSuccess(analyticsId);
                onClose();
            }, 1500);
        } catch (err) {
                ...
                {success ? (
                    <div className="p-8 text-center">
                        ...
                        <p className="text-on-surface-variant text-sm">
                            Data <span className="text-white font-medium">{analyticsName || analyticsId}</span> telah dihapus dari sistem.
                        </p>
                    </div>
                ) : (
                    <>
                        ...
                        <div className="px-6 pt-6 pb-4 text-center">
                            ...
                            <h3 className="text-lg font-semibold text-white mb-1">Hapus Data Analitik?</h3>
                            <p className="text-on-surface-variant text-sm leading-relaxed">
                                Apakah Anda yakin ingin menghapus{" "}
                                <span className="text-white font-medium">{analyticsName || analyticsId}</span>?
