...
/**
 * DeleteDasboard — Modal konfirmasi hapus widget/konfigurasi dashboard.
 *
 * Props:
 *  - dashboardItemId   (string)   : Setting key yang akan dihapus
 *  - dashboardItemName (string)   : Nama widget untuk tampilan (opsional)
 ...
 */
const DeleteDasboard = ({ dashboardItemId, dashboardItemName, isOpen, onClose, onSuccess }) => {
    ...
    const handleDelete = async () => {
        ...
        try {
            const response = await fetch(`${API_BASE}/settings/${dashboardItemId}`, {
                method: "DELETE",
            });
            ...
            if (!response.ok || !result.success) {
                throw new Error(result.error || `Gagal menghapus. Status: ${response.status}`);
            }
            ...
            setTimeout(() => {
                setSuccess(false);
                setError(null);
                if (onSuccess) onSuccess(dashboardItemId);
                onClose();
            }, 1500);
        } catch (err) {
            setError(err.message || "Terjadi kesalahan saat menghapus widget.");
        } finally {
            setDeleting(false);
        }
    };
                ...
                {success ? (
                    <div className="p-8 text-center">
                        ...
                        <p className="text-on-surface-variant text-sm">
                            Widget <span className="text-white font-medium">{dashboardItemName || dashboardItemId}</span> telah dihapus.
                        </p>
                    </div>
                ) : (
                    <>
                        ...
                        <div className="px-6 pt-6 pb-4 text-center">
                            ...
                            <h3 className="text-lg font-semibold text-white mb-1">Hapus Widget Dashboard?</h3>
                            <p className="text-on-surface-variant text-sm leading-relaxed">
                                Anda akan menghapus widget{" "}
                                <span className="text-white font-medium">{dashboardItemName || dashboardItemId}</span>.
